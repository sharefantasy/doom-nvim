...
# 并行进程数（默认为CPU核心数的1/3，避免资源耗尽）
MAX_JOBS=${MAX_JOBS:-$(( ($(nproc 2>/dev/null || sysctl -n hw.ncpu 2>/dev/null || echo 4) + 2) / 3 ))}
MAX_JOBS=$(( MAX_JOBS < 1 ? 1 : MAX_JOBS ))
MAX_JOBS=$(( MAX_JOBS > 4 ? 4 : MAX_JOBS ))  # 最大限制为4个进程
...
# 使用xargs进行并行编译，限制并发数
printf '%s\n' "${FNL_FILES[@]}" | xargs -P "$MAX_JOBS" -I {} bash -c '
    fnl_file="{}"
    lua_file="${fnl_file/'$FNL_DIR'/'$LUA_DIR'}"
    lua_file="${lua_file%.fnl}.lua"
