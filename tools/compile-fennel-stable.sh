...
mapfile -t FNL_FILES < <(find "$FNL_DIR" -name "*.fnl" -type f | sort)
    ...
    while [[ $retry_count -lt $max_retries ]]; do
        ...
        " 2>/dev/null; then
            ...
            if [[ $retry_count -lt $max_retries ]]; then
                echo -e "${YELLOW}⚠️  重试 $retry_count/$max_retries: $(basename "$fnl_file")${NC}"
                sleep 0.5
            fi
        fi
    done
    
    echo -e "${RED}❌ 编译失败: $(basename "$fnl_file")${NC}"
    ((FAILED_FILES++))
    return 1
}
...
for fnl_file in "${FNL_FILES[@]}"; do
    compile_fennel_with_retry "$fnl_file"
done
