...
char_counter.settings = {
    popup = {
        ...
        border = {padding = {top = 2, bottom = 2, left = 3, right = 3}},
        style = "rounded",
        enter = true,
...
char_counter.packages = {
    ["nui.nvim"] = {"MunifTanjim/nui.nvim", cmd = {"CountPrint"}}
}
...
char_counter.autocmds = {
    {
        "InsertEnter", "*", function()
            ...
            if vim.bo.buftype == "" then
                ...
                char_counter._insert_enter_char_count =
                    char_counter._get_current_buffer_char_count()
            end
        end
    }, {
        "InsertLeave", "*", function()
            ...
            if vim.bo.buftype == "" and char_counter._insert_enter_char_count then
                ...
                char_counter._accumulated_difference =
                    char_counter._accumulated_difference + diff
                print(("Accumulated difference %s"):format(
                          char_counter._accumulated_difference))
            end
        end
    }
...
char_counter.cmds = {
    {
        "CountPrint", function()
            ...
            local msg =
                ("char_counter: You have typed %s characters since I started counting."):format(
                    char_counter._accumulated_difference)
