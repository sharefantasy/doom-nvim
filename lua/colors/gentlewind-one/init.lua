...
-- Based On:        https://github.com/romgrk/gentlewind-one.vim and the original Gentlewind Emacs one
-- Helpers {{{
local gentlewind_one = {}

local utils = require("colors.utils")
local config = require("colors.gentlewind-one.config")
...
local function high_clear(group) vim.api.nvim_command("hi! clear " .. group) end
...
local function apply_highlight(groups)
    for group, styles in pairs(groups) do highlight(group, styles) end
end

-- Change the colorscheme colors depending on the current background, defaults to
-- gentlewind-one dark variant colors
...
gentlewind_one.load_colorscheme = function()
    -- General UI {{{
    ...
    local general_ui = {
        ...
        TabLine = {fg = base7, bg = bg_alt, gui = "bold"},
    ...
    if vim.opt.pumblend == 1 then vim.opt.pumblend = 20 end
    ...
    for key, _ in pairs(text_colors) do
        apply_highlight({["Text" .. key] = {fg = text_colors[key]}})
        apply_highlight({
            ["Text" .. key .. "Bold"] = {fg = text_colors[key], gui = "bold"}
        })
    end
    ...
    if configuration.plugins_integrations.bufferline and transparent_bg then
        ...
        --       https://github.com/NTBBloodbath/gentlewind-one.nvim/issues/8#issuecomment-883737667
    ...
    if configuration.plugins_integrations.vim_illuminate then
        local illuminated = {
            illuminatedWord = {cterm = "underline", gui = "underline"}
        }
