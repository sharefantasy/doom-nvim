...
doom = {
    ...
    use_package = function(...)
        ...
        doom.packages =
            vim.tbl_filter(package_override_predicate, doom.packages)

        for _, packer_spec in ipairs(arg) do
            table.insert(doom.packages, type(packer_spec) == "string" and
                             {packer_spec} or packer_spec)
        end
    end,
    ...
    use_keybind = function(...)
        local arg = {...}
        for _, bind in ipairs(arg) do table.insert(doom.binds, bind) end
    end,
