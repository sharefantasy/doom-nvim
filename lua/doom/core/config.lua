...
config.load = function()
    ...
    for section_name, section_modules in pairs(enabled_modules) do
        for _, module_name in pairs(section_modules) do
            ...
            local profiler_message = ("modules|import `%s.%s`"):format(
            ...
            if ok then
                ...
                log.error(string.format(
                              ...
                              section_name, module_name, result))
            end
    ...
    local ok, err = xpcall(dofile, function(err) debug.traceback(err) end,
                           config.source)
    local log = require("doom.utils.logging")
    ...
    if doom.auto_comment then vim.opt.formatoptions:append("croj") end
    if doom.movement_wrap then vim.cmd("set whichwrap+=<,>,[,],h,l") end
    ...
    if doom.global_statusline then vim.opt.laststatus = 3 end

    -- Use system clipboard
    if doom.clipboard then vim.opt.clipboard = "unnamedplus" end
    ...
    -- Color column
    vim.opt.colorcolumn = type(doom.max_columns) == "number" and
                              tostring(doom.max_columns) or ""
