...
python.settings = {
  ...
  --- @type string
  diagnostics_package = "ruff",
  --- String to access the null_ls diagnositcs provider
  --- @type string
  diagnostics_provider = "builtins.diagnostics.ruff",
...
python.autocmds = {
  {
    ...
    langs_utils.wrap_language_setup("python", function()
      if not python.settings.disable_lsp then
        langs_utils.use_lsp_mason(python.settings.lsp_name, { config = python.settings.lsp_config })
      end
