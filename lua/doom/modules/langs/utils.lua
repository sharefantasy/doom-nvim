...
module.use_null_ls_source = function(sources)
    ...
    for _, source in ipairs(sources) do
        ...
        local methods = type(source.method) == "string" and source.method or
                            table.concat(source.method, " ")
...
module.use_null_ls = function(package_name, null_ls_path, configure_function)
    ...
    if doom.features.linter then
        ...
        if not ok then require("lazy").load({plugins = {"null-ls.nvim"}}) end
        ...
        if doom.features.auto_install and package_name ~= nil then
            ...
            vim.defer_fn(function() start_null_ls() end, 1)
        end
    end
...
module.use_mason_package =
    function(package_name, success_handler, error_handler)
        ...
        local ok, err = xpcall(function()
            ...
            if not package:is_installed() then
                ...
                package:on("install:failed", function(pkg)
                    ...
                    if pkg and pkg.stdio and pkg.stdio.buffers and
                        pkg.stdio.buffers.stderr then
        ...
        if not ok then
            profiler.stop("mason|using package " .. package_name)
            on_err(package_name,
                   "There was an unknown error when installing.  Reason: \n" ..
                       err)
        end
    end
...
module.use_lsp_mason = function(lsp_name, options)
    ...
    if not utils.is_module_enabled("features", "lsp") then return end
    ...
    if opts.config then
        user_config = type(opts.config) == "function" and opts.config() or
                          opts.config
    end
    ...
    local start_lsp = function()
        local final_config = vim.tbl_deep_extend("keep", user_config or {},
                                                 capabilities_config)
        if lsp[config_name].setup == nil then
            log.warn(
                ("Cannot start LSP %s with config name %s. Reason: The LSP config does not exist, please create an issue so this can be resolved."):format(
                    lsp_name, config_name))
            return
        end
        lsp[config_name].setup(final_config)
        local lsp_config_server = lsp[config_name]
        if lsp_config_server.manager then
            local buffer_handler = lsp_config_server.filetypes and
                                       lsp_config_server.manager.try_add_wrapper or
                                       lsp_config_server.manager.try_add
    ...
    if utils.is_module_enabled("features", "auto_install") and
        not opts.no_installer then
