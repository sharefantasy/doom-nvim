...
lsp.settings = {
    ...
    sorting = {
        "offset", "exact", "score", "kind", "sort_text", "length", "order"
    }
}
...
lsp.configs["nvim-lspconfig"] = function()
    ...
    for severity, icon in pairs(signs) do
        ...
        vim.fn.sign_define(highlight,
                           {text = icon, texthl = highlight, numhl = highlight})
    end
    ...
    cmp.setup(vim.tbl_deep_extend("force",
        ...
        formatting = {
            format = function(entry, item)
                item.kind = string.format("%s %s", doom.features.lsp.settings
                                              .completion.kinds[item.kind],
                                          item.kind)
                item.dup = vim.tbl_contains({"path", "buffer"},
                                            entry.source.name)
                return item
            end
        },
        mapping = {
            ...
            ["<S-Tab>"] = cmp.mapping(function(fallback)
                ...
                elseif luasnip.jumpable(-1) then
                    vim.fn.feedkeys(
                        replace_termcodes("<Plug>luasnip-jump-prev"), "")
                else
                    fallback()
                end
            end, {"i", "s"})
        ...
        mapping = type(doom.features.lsp.settings.completion.mapping) ==
            "function" and doom.features.lsp.settings.completion.mapping(cmp) or
            doom.features.lsp.settings.completion.mapping,
        enabled = function()
            return _doom.cmp_enable and
...
lsp.configs["lsp_signature.nvim"] = function()
    -- Signature help
    require("lsp_signature").setup(vim.tbl_deep_extend("force", doom.features
                                                           .lsp.settings
                                                           .signature, {
        handler_opts = {border = doom.border_style}
    }))
end
                    ...
                    {
                        {
                            "c",
                            function()
                                lsp.__completions_enabled =
                                    not lsp.__completions_enabled
                                local bool2str = require("doom.utils").bool2str
                                print(string.format("completion=%s", bool2str(
                                                        lsp.__completions_enabled)))
                            end,
