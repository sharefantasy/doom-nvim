...
local core_modules = {
    core = {"gentlewind", "nest", "treesitter", "reloader", "updater"}
}
modules.enabled_modules = vim.tbl_deep_extend("keep", core_modules,
                                              dofile(modules.source))
...
modules.load_modules = function()
    ...
    -- Handle the Modules
    for section_name, _ in pairs(gentlewind.modules) do
        for module_name, module in pairs(gentlewind.modules[section_name]) do
            if type(module) ~= "table" then
                print(("Error on module %s type is %s val is %s"):format(
                          module_name, type(module), module))
            end
            local profile_msg = ("modules|init `%s.%s`"):format(section_name,
                                                                module_name)
            ...
            if module.requires_modules then
                for _, dependent_module in ipairs(module.requires_modules) do
                    local dep_section_name, dep_module_name = unpack(vim.split(
                                                                         dependent_module,
                                                                         "%."))

                    if not gentlewind.modules[dep_section_name][dep_module_name] then
                        should_enable_module = false
                        logger.error(
                            ('Gentlewind module "%s.%s" depends on a module that is not enabled "%s.%s".  Please enable the %s module.'):format(
                                section_name, module_name, dep_section_name,
            ...
            if should_enable_module then
                ...
                if module.packages then
                    ...
                    if repo ~= nil and spec[1] == nil then
                        ...
                        if type(spec.commit) == "table" then
                            ...
                            spec.commit =
                                utils.pick_compatible_field(spec.commit)
                        end

                        -- Only pin dependencies if gentlewind.freeze_dependencies is true
                        spec.pin = spec.commit and gentlewind.freeze_dependencies
                ...
                if module.autocmds then
                    local autocmds = type(module.autocmds) == "function" and
                                         module.autocmds() or module.autocmds
                    for _, autocmd_spec in ipairs(autocmds) do
                        autocmds_service.set(autocmd_spec[1], autocmd_spec[2],
                                             autocmd_spec[3], autocmd_spec)
                    end
                end

                if module.cmds then
                    for _, cmd_spec in ipairs(module.cmds) do
                        commands_service.set(cmd_spec[1], cmd_spec[2],
                                             cmd_spec[3] or cmd_spec.opts)
                    end
                end
...
modules.handle_user_config = function()
    ...
    for _, cmd_spec in pairs(gentlewind.cmds) do
        commands_service.set(cmd_spec[1], cmd_spec[2],
                             cmd_spec[3] or cmd_spec.opts)
    end

    -- Handle extra user autocmds
    for _, autocmd_spec in pairs(gentlewind.autocmds) do
        autocmds_service.set(autocmd_spec[1], autocmd_spec[2], autocmd_spec[3],
                             autocmd_spec)
    end

    -- Handle extra user keybinds
    for _, keybinds in ipairs(gentlewind.binds) do
    ...
    if modules._needs_sync then
        vim.api.nvim_create_autocmd("User", {
            ...
            callback = function()
                local logger = require("gentlewind.utils.logging")
                logger.error(
                    "Gentlewind-nvim has been installed.  Please restart gentlewind-nvim.")
            end
        })
    end
end

modules.handle_lazynvim = function() require("lazy").setup(gentlewind.packages) end
