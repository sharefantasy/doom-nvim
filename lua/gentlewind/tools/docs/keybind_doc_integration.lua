...
--
-- @example
-- local keybind_doc_integration = require('gentlewind.tools.docs.keybind_doc_integration')
...
module.get_rows = function()
    ...
    for _, row in ipairs(module.data) do table.insert(result, row) end
    return result
end
...
local pad_left = function(string, length, char)
    ...
    if difference > 0 then for _ = 1, difference do string = c .. string end end
    return string
end
...
local get_max_lengths = function(rows)
    local max_lengths = {}
    for _, _ in ipairs(rows[1]) do table.insert(max_lengths, 0) end

    for _, row in ipairs(rows) do
        for index, cell in ipairs(row) do
            print(index .. "| " .. cell .. " length is " .. string.len(cell) ..
                      " vs " .. max_lengths[index])
...
module.print_markdown = function()
    ...
    for index, cell in ipairs(module.header) do
        header_line = header_line ..
                          string.format(" %s |",
                                        pad_left(cell, max_lengths[index]))
        divider_line = divider_line ..
                           string.format(" %s |",
                                         pad_left("", max_lengths[index], "-"))
    end
    ...
    for _, row in ipairs(module.data) do
        ...
        for index, cell in ipairs(row) do
            line = line ..
                       string.format(" %s |", pad_left(cell, max_lengths[index]))
        end
