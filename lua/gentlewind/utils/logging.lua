...
local round = require("gentlewind.utils").round
...
log.new = function(config, standalone)
    ...
    local console_output = vim.schedule_wrap(
            ...
            local message = {
                ("[%s] %s"):format(config.plugin, console_string),
                level_config.hl
            }
    ...
    local log_at_level = function(level, level_config, message_maker, ...)
        ...
        if config.use_file then
            ...
            if not fp then
                vim.cmd(string.format([[echom "Couldn't open log file [%s]"]],
                                      outfile))
            else
