...
function async:new(job_opts)
    ...
    for opt, val in pairs(job_opts) do self[opt] = val end
...
function async.options()
    ...
    if async.cwd then options.cwd = async.cwd end

    if async.env then options.env = async.env end

    if async.detach then options.detach = async.detach end
...
async.shutdown = function(code, signal)
    if async.on_exit then async.on_exit(code, signal) end
    if async.on_stdout then async.stdout:read_stop() end
    if async.on_stderr then async.stderr:read_stop() end
    async.stop()
end
