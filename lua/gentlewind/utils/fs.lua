...
fs.get_seperator = function()
    if fs.is_windows and not fs.use_shellslash then return "\\" end
    return "/"
end

--- Joins a number of strings into a valid path
---@vararg string String segments to convert to file system path
fs.join_paths = function(...) return table.concat({...}, fs.get_seperator()) end
...
fs.rm_dir = function(path)
    ...
    if type(handle) == "string" then return fs.notify.error(handle) end
