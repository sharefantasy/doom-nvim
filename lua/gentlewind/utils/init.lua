...
--- Gentlewind Nvim version
utils.version = {major = 4, minor = 1, patch = 0}

--- Currently supported version of neovim for this build of gentlewind-nvim
utils.nvim_latest_supported = "nvim-0.8"

utils.gentlewind_version = string.format("%d.%d.%d", utils.version.major,
                                   utils.version.minor, utils.version.patch)

-- Finds `filename` (where it is a gentlewind config file).
utils.find_config = function(filename)
    local function get_filepath(dir)
        return table.concat({dir, filename}, system.sep)
    end

    local path = get_filepath(system.gentlewind_configs_root)
    if fs.file_exists(path) then return path end
    path = get_filepath(system.gentlewind_root)
    if fs.file_exists(path) then return path end
    local candidates = vim.api.nvim_get_runtime_file(
                           get_filepath("*" .. system.sep .. "doon-nvim"), false)
    if not vim.tbl_isempty(candidates) then return candidates[1] end
    -- TODO: Consider copying the default to the user config dir.
    -- Can't use log yet, gentlewind doesn't exist.
...
utils.bool2num = function(bool_or_num)
    if type(bool_or_num) == "boolean" then return bool_or_num and 1 or 0 end
    return bool_or_num
end
...
utils.bool2str = function(bool) return bool and "on" or "off" end
...
utils.safe_require = function(path)
    ...
    log.debug(string.format("Gentlewind: loading '%s'... ", path))
    local ok, result = xpcall(require, function(err) debug.traceback(err) end,
                              path)
...
utils.get_sysname = function() return vim.loop.os_uname().sysname end
...
--- Check if the given plugin is disabled in gentlewind-nvim/modules.lua
...
utils.pick_compatible_field = function(compatibility_table)
    ...
    for _, version in ipairs(sorted) do
        ...
        local ver = version == "latest" and utils.nvim_latest_supported or
                        version
    ...
    if last_field == nil then error("Error getting compatible field.") end
    return last_field
end
