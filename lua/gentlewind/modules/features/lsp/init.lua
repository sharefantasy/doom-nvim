...
lsp.settings = {
  snippets = { history = true, updateevents = "TextChanged,TextChangedI" },
  signature = {
    bind = true,
    doc_lines = 10,
...
lsp.configs["nvim-lspconfig"] = function()
  -- Lsp Symbols
  local icons = {
    [vim.diagnostic.severity.ERROR] = gentlewind.features.lsp.settings.icons.error,
    [vim.diagnostic.severity.WARN] = gentlewind.features.lsp.settings.icons.warn,
    [vim.diagnostic.severity.INFO] = gentlewind.features.lsp.settings.icons.info,
    [vim.diagnostic.severity.HINT] = gentlewind.features.lsp.settings.icons.hint,
  }
  ...
  vim.diagnostic.config {
    virtual_text = gentlewind.features.lsp.settings.virtual_text,
    severity_sort = gentlewind.features.lsp.settings.severity_sort,
    float = { show_header = false, border = "rounded" },
    globals = { "vim", "require" },
  }
  -- Border for lsp_popups
  vim.lsp.handlers["textDocument/hover"] = vim.lsp.with(vim.lsp.handlers.hover, { border = gentlewind.border_style })
  vim.lsp.handlers["textDocument/signatureHelp"] =
    vim.lsp.with(vim.lsp.handlers.signature_help, { border = gentlewind.border_style })
  -- symbols for autocomplete
  local kinds = {}
  for typ, icon in pairs(gentlewind.features.lsp.settings.completion.kinds) do
...
lsp.configs["nvim-cmp"] = function()
  ...
  luasnip.config.set_config(gentlewind.features.lsp.settings.snippets)
  ...
  if _gentlewind.cmp_enable == nil then
    _gentlewind.cmp_enable = true
  end

  -- Fetch the comparators from cmp
  local comparators = require "cmp.config.compare"
  gentlewind.features.lsp.settings.sorting = vim.tbl_map(function(comparator)
    return comparators[comparator]
  end, gentlewind.features.lsp.settings.sorting)

  cmp.setup(vim.tbl_deep_extend("force", gentlewind.features.lsp.settings.completion, {
    completeopt = nil,
    completion = {
      completeopt = gentlewind.features.lsp.settings.completion.completeopt,
    },
    formatting = {
      format = function(entry, item)
        item.kind = string.format("%s %s", gentlewind.features.lsp.settings.completion.kinds[item.kind], item.kind)
    ...
    mapping = type(gentlewind.features.lsp.settings.completion.mapping) == "function"
        and gentlewind.features.lsp.settings.completion.mapping(cmp)
      or gentlewind.features.lsp.settings.completion.mapping,
    enabled = function()
      return _gentlewind.cmp_enable and vim.api.nvim_buf_get_option(0, "buftype") ~= "prompt"
    end,
  }))
end
lsp.configs["lsp_signature.nvim"] = function()
  -- Signature help
  require("lsp_signature").setup(vim.tbl_deep_extend("force", gentlewind.features.lsp.settings.signature, {
    handler_opts = { border = gentlewind.border_style },
  }))
end
    ...
    { "<leader>", group = "prefix", {
            ...
            { "d", group = "diagnostics", {
                ...
                { "L", function()
                    vim.diagnostic.open_float(0, {
                      focusable = false,
                      border = gentlewind.border_style,
                    })
                  end, desc = "Line" },
        ...
        { "t", group = "tweak", {
            { "c", function()
                ...
                local bool2str = require("gentlewind.utils").bool2str
