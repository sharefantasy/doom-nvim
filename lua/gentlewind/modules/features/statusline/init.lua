...
statusline.state.finish_mason_package = function(name)
    ...
    statusline.state.installing_mason_packages =
        vim.tbl_filter(function(val) return val ~= name end, packages)
end
...
statusline._generate_colorscheme = function()
    local colors = vim.tbl_map(function(hl)
        return {hex = hl, rgb = hex2rgb(hl)}
    end, {
        statusline._safe_get_highlight("luaTSField", "TSField", "TSVariable",
                                       "Field", "Variable").foreground,
        statusline._safe_get_highlight("luaTSConditional", "TSConditional",
                                       "TSConstant", "Conditional", "Constant")
            .foreground,
        statusline._safe_get_highlight("luaTSFunction", "TSFunction", "Function")
            .foreground,
        statusline._safe_get_highlight("luaTSKeywordFunction",
                                       "TSKeywordFunction", "Function")
            .foreground,
        statusline._safe_get_highlight("luaTSString", "TSString", "String")
            .foreground,
        statusline._safe_get_highlight("luaTSNumber", "TSNumber", "Number")
            .foreground
    })
    ...
    local rate_color = function(hsv)
        ...
        local rating = (averageDistanceRating + proximityRating +
                           saturationRating) * allowedBrightnessRating
        return rating
    end
    ...
    for _, color in ipairs(colors) do color.rating = rate_color(color.hsv) end
...
statusline.configs["heirline.nvim"] = function()
    ...
    local special, special2, special3 = gentlewind.modules.features.statusline
                                            ._generate_colorscheme()

    local safe_get_highlight = gentlewind.modules.features.statusline
                                   ._safe_get_highlight
    local colors = {
        ...
        git = {
            ...
            change = safe_get_highlight("diffChanged", "DiffChange",
                                        "DiffChange").foreground
        }
    }

    local Notch = {
        {provider = function() return "█" end, hl = {fg = colors.special2}}
    }
  ...
  --]]
    ...
    FileBlock = utils.insert(FileBlock, FileSize, FileIcon,
    ...
    local MasonStatusElement = {
        condition = function()
            return #gentlewind.features.statusline.state.installing_mason_packages > 0
        end,
        provider = function()
            local installing_mason_packages =
                gentlewind.features.statusline.state.installing_mason_packages
            return (("Installing %s... "):format(table.concat(
                                                     installing_mason_packages,
                                                     ", ")))
        end,
        on_click = {callback = function() vim.cmd("Mason") end, name = "mason"},
        hl = {fg = colors.special}
    }
  ...
  --]]
    local GitBlock = {
        condition = function()
            local has_gitsigns_module = gentlewind.features.gitsigns ~= nil
    ...
    local Ruler = {
        {hl = {fg = colors.dim}, provider = " %P "}, {provider = "%3l:%-2c "}
    }
...
statusline.try_refresh = function()
    xpcall(gentlewind.modules.features.statusline.configs["heirline.nvim"],
           debug.traceback)
end
...
statusline.autocmds = {
    {
        "ColorScheme", "*",
        function()
            vim.defer_fn(function() statusline.try_refresh() end, 1)
        end
    }, -- Sometimes the colorscheme doesn't load on the first try
    {
        ...
        "*",
        function()
            vim.defer_fn(function() statusline.try_refresh() end, 50)
        end,
        once = true
    }
}
