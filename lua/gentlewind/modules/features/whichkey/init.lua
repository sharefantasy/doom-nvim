...
whichkey.settings = {
    ...
    win = {padding = {0, 0, 0, 0}, border = gentlewind.border_style},
...
whichkey.configs["which-key.nvim"] = function()
    vim.g.mapleader = gentlewind.features.whichkey.settings.leader

    local wk = require("which-key")

    wk.setup(gentlewind.features.whichkey.settings)
    ...
    for section_name, _ in pairs(gentlewind.modules) do
        for _, module in pairs(gentlewind[section_name]) do
    ...
    if gentlewind.binds and #gentlewind.binds >= 1 then
        keymaps_service.applyKeymaps(gentlewind.binds, nil, {whichkey_integration})
    end
end
