...
_G._gentlewind_reloader = _G._gentlewind_reloader ~= nil and _G._gentlewind_reloader or {}
...
local function path_to_lua_module(module_path)
    local lua_path = string.format("%s%slua", system.gentlewind_root, system.sep)

    -- Remove the Neovim config dir and the file extension from the path
    module_path = string.match(module_path, string.format("%s%s(.*)%%.lua",
                                                          utils.escape_str(
                                                              lua_path),
                                                          system.sep))
...
reloader.reload_lua_module = function(mod_path, quiet)
    if mod_path:find("/") then mod_path = path_to_lua_module(mod_path) end

    -- If gentlewind-nvim fails to reload, warn user once per session
    if mod_path == nil then
        if reloader.has_failed_reload == false then
            log.warn(
                "reloader: Failed to reload gentlewind config because this file is not in nvim config directory.  Is your gentlewind nvim config directory symlinked?")
...
reloader._reload_gentlewind = function()
    vim.cmd("hi clear")
    if vim.fn.exists(":LspRestart") ~= 0 then vim.cmd("silent! LspRestart") end

    -- Remember which modules/packages installed to check if user needs to `:PackerSync`
    local old_modules = require("gentlewind.core.modules").enabled_modules
    local old_packages = vim.tbl_map(function(t) return t[1] end, gentlewind.packages)

    -- Reset State
    require("gentlewind.services.commands").del_all()
    require("gentlewind.services.autocommands").del_all()
    require("gentlewind.services.profiler").reset()

    -- Unload gentlewind.modules/gentlewind.core lua files
    for k, _ in pairs(package.loaded) do
        -- TODO: add `gentlewind.user` here as well?
        if string.match(k, "^gentlewind%.core") or string.match(k, "^gentlewind%.modules") then
            package.loaded[k] = nil
        end
    end

    -- Reload core entry point
    reloader.reload_lua_module("gentlewind.core", true)
    -- Reload which modules are enabled
    reloader.reload_lua_module("gentlewind.core.modules", true)
    -- Prepare the enabled modules, reload the user config.lua
    reloader.reload_lua_module("gentlewind.core.config", true)
    require("gentlewind.core.config"):load()
    -- Install, bind, add autocmds etc for all modules and user configs
    require("gentlewind.core.modules"):load_modules()
    require("gentlewind.core.modules"):handle_user_config()
    -- VimEnter to emulate loading neovim

    local modules = require("gentlewind.core.modules").enabled_modules
    local packages = vim.tbl_map(function(t) return t[1] end, gentlewind.packages)
    local needs_install = vim.deep_equal(modules, old_modules) and
                              vim.deep_equal(packages, old_packages)
    if needs_install then
        if not _G._gentlewind_reloader._has_shown_packer_compile_message then
            log.warn(
                "reloader: You will have to run `:Lazy build` before changes to plugin configs take effect.")
            _G._gentlewind_reloader._has_shown_packer_compile_message = true
        end
    else
        log.warn(
            "reloader: Run `:Lazy sync` to install and configure new plugins.")
    end
...
reloader.reload = function()
    -- Store the time taken to reload Gentlewind
    local reload_time = vim.fn.reltime()
    log.info("Reloading Gentlewind ...")

    --- Reload Neovim configurations
    reloader._reload_gentlewind()

    log.info("Reloaded Gentlewind in " ..
                 vim.fn
                     .printf("%.3f",
...
reloader.settings = {reload_on_save = false}
reloader.packages = {}
reloader.configs = {}

reloader.cmds = {{"GentlewindReload", function() reloader.reload() end}}

reloader.autocmds = function()
    local autocmds = {}

    -- RELOAD GENTLEWIND ON SAVE
    if reloader.settings.reload_on_save then
        table.insert(autocmds, {
            "BufWritePost", "*/gentlewind/**/*.lua,*/user/**/*.lua", reloader.reload
        })
        table.insert(autocmds, {
            "BufWritePost", "*/modules.lua,*/config.lua", function()
