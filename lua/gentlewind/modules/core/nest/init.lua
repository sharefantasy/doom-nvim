...
mapper.packages = {["nvim-mapper"] = {"lazytanuki/nvim-mapper"}}

-- TODO: Not happy with how messy the integrations are, refactor!
mapper.configs = {}
mapper.configs["nvim-mapper"] = function()
    require("nvim-mapper").setup(gentlewind.core.gentlewind.settings.mapper)
    ...
    local get_mapper_integration = function()
        ...
        local determine_uid = function(lhs, name, mode)
            ...
            if unique_id_table[n] == lhs then return nil end
        ...
        mapper_integration.handler = function(node, node_settings)
            if node.name == nil then return end
            ...
            local category =
                node.category or get_category_for_command(node.lhs) or "unknown"
            ...
            if category == nil or description == nil then return end

            for mode in string.gmatch(node_settings.mode, ".") do
                local sanitizedMode = mode == "_" and "" or mode

                local id = node.uid or
                               determine_uid(node.lhs, node.name, sanitizedMode)

                if id ~= nil then
                    local rhs = type(node.rhs) == "function" and "<function>" or
                                    node.rhs
                    if node_settings.buffer then
                        Mapper.map_buf_virtual(sanitizedMode, node.lhs, rhs,
                                               node_settings.options, category,
                                               id, description)
                    else
                        Mapper.map_virtual(sanitizedMode, node.lhs, rhs,
                                           node_settings.options, category, id,
                                           description)
                    end
                end
            end
