...
--  Gentlewind updater
--
--  Update your gentlewind nvim config using the :GentlewindUpdate command.
    ...
    Job:new({
        ...
        on_exit = function(j)
            ...
            local filter_predicate = function(version)
                local is_unsupported = version:match("^1") or
                                           version:match("v2") or
                                           version:match("v3")
                if is_unsupported or
                    (not updater.settings.unstable and
                        is_version_unstable(version)) then
                    return false
                end
                return true
            end
...
updater._get_last_version_for_commit = function(commit_sha, callback)
    ...
    Job:new({
        command = "git",
        args = {
            "tag", "-l", "--sort", "-version:refname", "--merged", commit_sha
        },
        cwd = updater._cwd,
        on_exit = function(j, exit_code)
            if exit_code ~= 0 then
            ...
            local try_compare_updates = function()
                if cur_version and all_versions then
                    ...
                    if #all_versions > 0 then
                        ...
                        callback(nil, nil,
                                 "Error getting latest version.  The versions list is empty!")
                    end
                end
            end
...
--- Entry point for `:GentlewindCheckUpdates`, fetches new tags, compares with current version and notifies results
...
updater.check_updates = function(quiet)
    ...
    if not quiet then vim.notify("updater: Checking updates...") end

    updater._fetch_current_and_latest_version(
        function(current_version, latest_version, error)
            vim.defer_fn(function()
                if error then
                    log.error(
                        ("updater: Error checking updates... %s"):format(error))
                    return
                end

                if current_version == latest_version then
                    if not quiet then
                        vim.notify(("updater: You are up to date! (%s)"):format(
                                       current_version))
                    end
                else
                    vim.notify(
                        ("updater: There is a new version (%s).  You are currently on %s.  Run `:GentlewindUpdate` to update."):format(
...
updater._try_merge_version = function(target_version, callback)
    ...
    local merge_job = Job:new({
        ...
        on_exit = function(j, exit_code)
            if exit_code ~= 0 then
                callback("Error merging " .. target_version ..
...
--- Entry point for `:GentlewindUpdate`, fetches new tags, compares with current version and attempts to merge new tags into current branch
updater._try_update = function()
    ...
    updater._get_branch_name(function(branch_name, error)
        ...
        elseif branch_name == "next" or branch_name == "main" then
            error_message =
                "You cannot use `:GentlewindUpdate` from within the `main` branch.  Please make a new branch for your custom config (`git checkout -b my-config`)."
        end
            ...
            function(current_version, latest_version, get_version_error)
                vim.defer_fn(function()
                    if get_version_error then
                        log.error(
                            ("updater: Error checking updates... %s"):format(
                                get_version_error))
                        return
                    end
                    ...
                    if current_version == latest_version then
                        vim.notify(
                            ("updater: You are already using the latest version! (%s)"):format(
                                current_version))
                    else
                        -- Attempt to merge new version into user's custom config branch
                        updater._try_merge_version(latest_version,
                                                   function(merge_error)
                            vim.defer_fn(function()
                                if merge_error then
                                    log.error(
                                        ("updater: Error updating... %s"):format(
                                            merge_error))
                                else
                                    local message =
                                        ("updater: Updated to version %s!"):format(
                                            latest_version)
                                    -- Only print changelog info if it's a stable release
                                    if not is_version_unstable(latest_version) then
                                        message = message ..
                                                      ("  Check the changelog at https://github.com/gentlewind-neovim/gentlewind-nvim/releases/tag/%s"):format(
                                                          latest_version)
                                    end
                                    vim.notify(message)
                                end
...
updater.cmds = {
    {"GentlewindUpdate", function() updater._try_update() end},
    {"GentlewindCheckUpdates", function() updater.check_updates(false) end}
}
return updater
