...
vue.settings = {
    ...
    --- @type string|string[]
    treesitter_grammars = {
        "vue", "css", "scss", "html", "scss", "javascript", "typescript"
    },
...
vue.autocmds = {
    {
        ...
        langs_utils.wrap_language_setup("vue", function()
            if not vue.settings.disable_lsp then
                ...
                local function get_typescript_server_path(root_dir)
                    ...
                    local function check_dir(path)
                        tsdk_path = lspconfig_util.path.join(path,
                                                             "node_modules",
                                                             "typescript", "lib")
                        local tsserverlibrary_path =
                            lspconfig_util.path.join(tsdk_path,
                                                     "tsserverlibrary.js")
                ...
                local volar_root_dir = lspconfig_util.root_pattern(
                                           "package.json")

                -- Contains base configuration necessary for volar to start
                local build_base_config =
                    function(volar_handle)
                        local fallback_sdk_path =
                            lspconfig_util.path.join(
                                volar_handle:get_install_path(), "node_modules",
                                "typescript", "lib")
                        return {
                            default_config = {
                                ...
                                on_new_config = function(new_config,
                                                         new_root_dir)
                                    if new_config.init_options and
                                        new_config.init_options.typescript and
                ...
                langs_utils.use_mason_package("vue-language-server",
                                              function(handle)
                    local lsp_config = vim.tbl_deep_extend("force", {},
                                                           gentlewind.langs.vue
                                                               .settings
                                                               .lsp_config,
                                                           build_base_config(
                                                               handle))
                    langs_utils.use_lsp_mason("volar", {config = lsp_config})
                end)
            end
            ...
            if not vue.settings.disable_diagnostics then
                langs_utils.use_null_ls(vue.settings.diagnostics_package,
                                        vue.settings.diagnostics_provider,
                                        vue.settings.diagnostics_config)
            end
            if not vue.settings.disable_code_actions then
                langs_utils.use_null_ls(vue.settings.code_actions_package,
