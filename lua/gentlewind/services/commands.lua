...
local data = _G._gentlewind_commands_service_data or {command_actions = {}}
_G._gentlewind_commands_service_data = data
...
local set_command_implementations = {
    ["nvim-0.5"] = function(name, command, opts)
        ...
        if type(command) == "string" then
            ...
            cmd_string = cmd_string ..
                             ("lua _gentlewind_commands_service_data.command_actions[%d]"):format(
                                 uid)
...
local del_command_implementations = {
    ["nvim-0.5"] = function(name) vim.cmd(("delcommand %s"):format(name)) end,
    ["nvim-0.8"] = function(name) vim.api.nvim_del_user_command(name) end
}
...
commands_service.del_all = function()
    for name, _ in pairs(commands_service.stored_names) do
        if name then del_command_fn(name) end
    end
    commands_service.stored_names = {}
end
