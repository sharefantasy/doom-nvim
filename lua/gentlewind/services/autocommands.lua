...
--- Manages references to all commands to be cleared for :GentlewindReload
-- TYPES
...
local data = _G._gentlewind_autocmds_service_data or {
...
_G._gentlewind_autocmds_service_data = data

-- Store all autocommands inside of an augroup for gentlewind-nvim
if vim.fn.has("nvim-0.8") then
    vim.api.nvim_create_augroup("GentlewindAutoCommands", {clear = true})
else
    vim.cmd([[
    augroup GentlewindAutoCommands
      autocmd!
    augroup END
  ]])
end
...
local set_autocmd_implementations = {
    ["nvim-0.5"] = function(event, pattern, action, opts)
        ...
        if opts.nested then cmd_string = cmd_string .. "++nested " end
        if opts.once then cmd_string = cmd_string .. "++once " end
        ...
        if type(action) == "string" then
            ...
            cmd_string = cmd_string ..
                             (":lua _gentlewind_autocmds_service_data.autocmd_actions[%d]()"):format(
                                 uid)
        end
        vim.cmd(cmd_string)
        return uid
...
local del_autocmd_implementations = {
    ["nvim-0.5"] = function(id)
        ...
        if delete_signature then vim.cmd(delete_signature) end
    end,
    ["latest"] = function(id) vim.api.nvim_del_autocmd(id) end
}
...
local del_all_autocmd_fn = utils.pick_compatible_field(
                               del_all_autocmd_implementations)
...
autocmds_service.del_all = function() del_all_autocmd_fn() end
