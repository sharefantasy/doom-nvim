...
-- 默认关闭 profiler（降低启动期开销）；需要时设置环境变量 GENTLEWIND_PROFILE=1
local enabled = vim.env.GENTLEWIND_PROFILE == "1"
...
profiler.log = function(options)
    ...
    local chunks = vim.tbl_map(function(key)
        return vim.tbl_extend("force", {key = key, depth = 1},
                              profiler.chunks[key])
    end, vim.tbl_keys(profiler.chunks))

    table.sort(chunks, function(a, b) return a.start < b.start end)
    ...
    for _, parent in ipairs(chunks) do
        ...
        if #children > 0 then parent.has_children = true end
        vim.tbl_map(function(child) child.depth = child.depth + 1 end, children)
    end

    --- Print command information
    vim.api.nvim_echo({
        {
            "This shows the time spent initialising the gentlewind-nvim framework.\n",
            "Comment"
        }, {
    ...
    vim.api.nvim_echo({
        ...
        {utils.right_pad("Task", 48), "Int"}, {"Duration(ms)", "Int"},
    ...
    for index, entry in ipairs(chunks) do
        ...
        local name_or_category, name_or_nil = unpack(
                                                  vim.split(entry.key, "|", {}))
        ...
        if not is_async or (show_async and is_async) then
            if name_or_category and name_or_nil then
                table.insert(echo_tbl, {
                    utils.right_pad(name_or_category, 12, " "), "Comment"
                })
                table.insert(echo_tbl, {
                    string.rep(" │", depth - 1) ..
                ...
                table.insert(echo_tbl, {
                    utils.right_pad(name_or_nil, 48 - depth * 2, " "), "Normal"
                })
            else
                table.insert(echo_tbl, {
                    string.rep("  ", depth - 1) ..
            ...
            local stop_time = entry.stop and
                                  ("%.03f"):format(
                                      (entry.stop - profiler.all_start) / 1e6) or
                                  "Hasn't stopped yet"

            table.insert(echo_tbl, {
                ("\t\t%.03f -> %s"):format(start_time, stop_time), "Comment"
            })
...
profiler.reset = function() profiler.chunks = {} end
