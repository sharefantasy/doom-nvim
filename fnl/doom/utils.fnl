;; doom.utils
;; Utility functions for doom-nvim
...
(fn is_module_enabled [section module-name]
  ...
  (and doom.modules
       doom.modules[section]
       doom.modules[section][module-name]))
...
(fn find_config [filename]
  ...
  (local search-paths [(.. config-dir "/" filename)
                       (.. config-dir "/../doom-nvim/" filename)
                       (.. data-dir "/site/pack/packer/start/doom-nvim/" filename)])
...
(fn pick_compatible_field [version-table]
  ...
  (each [version value (pairs version-table)]
    (lua :return value)))
...
(fn logging.debug [message]
  "Log a debug message"
  (when (= doom.logging "debug")
    (vim.notify message vim.log.levels.DEBUG)))

{: safe_require
 : is_module_enabled
 : find_config
 : pick_compatible_field
 : logging}